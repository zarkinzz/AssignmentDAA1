 ...
 * Time: O(n log n). Sort by X, split, check strip by Y.
 * We use squared distance inside loops (sqrt only at the end).
 */
public final class ClosestPair {
    ...
    public static double closestDistance(Point[] pts, Metrics m) {
        ...
        m.incAlloc(); // one allocation for buffer

        double best2 = solve(px, py, 0, px.length - 1, strip, m); // squared
        return Math.sqrt(best2);
    }

    // Recursion on px[l..r]; py contains EXACTLY the same points sorted by Y.
    private static double solve(Point[] px, Point[] py, int l, int r, Point[] strip, Metrics m) {
        try (var d = m.guard()) {
            ...
            // Use identity set of left points to route ties (x == midX) correctly.
            ...
            // build strip near mid line (points with |x - midX| < delta), ordered by Y (py already)
            int sy = 0;
            for (Point p : py) {
                if (Math.abs(p.x - midX) < delta) strip[sy++] = p;
            }

            // scan strip: each point compares with next up to ~7 points
            for (int i = 0; i < sy; i++) {
